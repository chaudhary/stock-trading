      ...
      compile: function(elementTemplate, attr, linker) {
        return function($scope, element, $attr, controllers) {
          ...
          linker(tempScope = $scope.$new(), function(template) {
            ...
            tempScope.$destroy();
            var fix_top_padding = element.parent().offset().top;
            return adapter = {
              viewport: viewport,
              ...
              topDataPos: function() {
                return topPadding.paddingHeight() + fix_top_padding;
              }
            };
          });
          ...
          fetch = function(rid, scrolling) {
            ...
            if (direction) {
              if (buffer.length && !shouldLoadBottom()) {
                ...
                return datasource.get(next, bufferSize, function(result, no_more_data) {
              ...
              if (buffer.length && !shouldLoadTop()) {
                ...
                return datasource.get(first - bufferSize, bufferSize, function(result) {
                  ...
                  if (result.length < bufferSize) {
                    bof = true;
                    viewport.scrollTop(viewport.scrollTop() - adapter.topPadding());
                    adapter.topPadding(0);
                  }
                  if (result.length > 0) {
                    if (buffer.length) {
