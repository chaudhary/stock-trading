...
angular.module('angular-timeago').config(["timeAgoSettings", function(timeAgoSettings) {
...
angular.module('angular-timeago').factory('nowTime', ["$interval", "timeAgo", "timeAgoSettings", function($interval, timeAgo, timeAgoSettings) {
  var nowTime;
...
angular.module('angular-timeago').constant('timeAgoSettings', {
  refreshMillis: 1000,
  allowFuture: false,
  overrideLang: null,
...
angular.module('angular-timeago').directive('timeAgo', ["timeAgo", "nowTime", function(timeAgo, nowTime) {
  return {
    scope: {
      fromTime: '@',
      format: '@'
...
angular.module('angular-timeago').factory('timeAgo', ["$filter", "timeAgoSettings", function($filter, timeAgoSettings) {
  var service = {};
    ...
    if (!isNaN(fullDateAfterSeconds)) {
      ...
      if ((distanceMillis >= 0 && fullDateAfterMillis <= distanceMillis) ||
        (distanceMillis < 0 && fullDateAfterMillis >= distanceMillis)) {
          var year_short = $filter('date')(fromTime, "yy");
          var year = $filter('date')(fromTime, "yyyy");
          var date_str = [$filter('date')(fromTime, "d"), $filter('date')(fromTime, "MMM")].join(" ");
          if(year == $filter('date')(new Date(), "yyyy")){
            return date_str;
          }else if(year_short == $filter('date')(new Date(), "yy")){
            return date_str + " " + year;
          }else{
            return date_str + " " + year_short;
          }
      }
    }
...
angular.module('angular-timeago').filter('timeAgo', ["nowTime", "timeAgo", function(nowTime, timeAgo) {
